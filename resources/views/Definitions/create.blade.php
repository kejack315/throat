<x-app-layout>
    ...
    <section class="w-full p-6 flex flex-col gap-4">
        ...
        <form method="POST" action="{{ route('definitions.store') }}" class="flex flex-col w-full gap-4">
            @csrf
            @method('POST')
            ...
            <div class="flex flex-row gap-4 rounded-md bg-gray-200 dark:bg-gray-900">
                <label for="rating" class="p-2 w-1/6 rounded-l-md bg-gray-500 dark:bg-gray-800 text-gray-100">
                    {{ __("Rating") }}
                </label>
                <select id="stars" name="stars" class="p-2 w-5/6 bg-gray-200 dark:bg-gray-900 rounded-r-md" required>
                    @for ($i = 0; $i <= 10; $i++)
...
{{--                </label>--}}
{{--                <input id="definition" name="definition" type="text"--}}
{{--                       class="p-2 w-5/6 bg-gray-200 dark:bg-gray-900 rounded-r-md" value="{{ old('stars') }}"--}}
{{--                       required>--}}
{{--            </div>--}}
